...
def rain_machine(total_level_width, total_level_height):
    ...
    def create():
        if numpy.random.randint(100) < 50:
            c = cm[numpy.random.choice(list(range(len(cm))))]
...
class Emitter(object):
    ...
    def update(self):
        tmp = itertools.chain(self.particles, *list(map(next, self.factories))) 
        tmp = filter(Particle.move, tmp) # side effect!
        self.particles = list(tmp)

    def draw(self, screen, pos):
        x, y = pos#_translater_func(self.pos)
